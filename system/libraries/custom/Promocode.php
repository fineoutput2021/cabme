        ...
        if (!empty($promocode_data)) {
            ...
            if ($promocode_data[0]->ptype==1) {
                ...
                if (empty($promocodeAlreadyUsed)) {
                    // echo strtotime($promocode_data[0]->end_date)." ".$cur_date." ".strtotime($promocode_data[0]->start_date);die();
                    if (strtotime($promocode_data[0]->end_date) >= $cur_date && strtotime($promocode_data[0]->start_date) <= $cur_date) {
                        ...
                        if ($promocode_data[0]->type==1) { //-- Discount in percentage
                          $discount_amt = $order_data[0]->total_amount * $promocode_data[0]->percentage_amount/100;
                            ...
                            $discount = $order_data[0]->total_amount - $promocode_data[0]->percentage_amount;
                        }
                        }   //endif of minorder
                        else {
                            $respone['status'] = false;
                            $respone['message'] = 'Please add more products for promocode';
            ...
            if (!empty($last_id)) {
                ...
                $respone['status'] = false;
                $respone['message'] = 'some eroor occurred! please try again';
                return json_encode($respone);
                exit;
            }
        } else {
