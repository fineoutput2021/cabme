...
class CI_Booking
{
  ...
  public function ViewSelfDriveCars($receive)
  {
    ...
    if ($receive['sort'] != 'none') {
      ...
      $self_cars = $this->CI->db->order_by('is_available', 'asc')->get_where('tbl_selfdrive', array('city_id' => $receive['city_id'], 
       'is_active' => 1))->result();
    }
    //     $links='';
    // }
  ...
  public function ViewOutstationCars($receive)
  {
    if ($receive['sort'] != 'none') {
      ...
      $outstation_cars = $this->CI->db->order_by('is_available', 'asc')->get_where('tbl_outstation', array('city_id' => $receive['city_id'],  'is_active' => 1))->result();
    }
    $car_data = [];
